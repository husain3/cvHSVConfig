...
int main(int argc, const char * argv[]) {
    ...
    while(1)
    {
        ...
        cameraFeed = imread("stop-sign-belmead2.png");

	Mat imageBrighnessHigh;
	cameraFeed.convertTo(imageBrighnessHigh, -1, 1, 50);
        
        //convert frame from BGR to HSV colourspace
        cvtColor(imageBrighnessHigh, HSV, CV_BGR2HSV);
        //filter HSV image between values and store filters image to threshold matrix
        
        inRange(imageBrighnessHigh, Scalar(H_MIN, S_MIN, V_MIN), Scalar(H_MAX, S_MAX, V_MAX), threshold);
        
        //perform morphological operations on threshold image to eliminate noise and emphasize the filtered objects
        ...
        imshow(windowName, imageBrighnessHigh);
